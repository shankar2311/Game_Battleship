...
class Board:
    ...
    def add_ships(self):
        ...
        Ships in player board are visible and in computer board are not shown
    ...
    def new_game():
        ...
        while player.score < 4 and computer.score < 4:
            ...
            while True:
                p_rw_cl = player.player_guess()

                if computer.board[p_rw_cl[0]][p_rw_cl[1]] not in ['X', 'O']:

                    player.check_guess(p_rw_cl, computer)
                    computer.print_board()
                    break
                else:
